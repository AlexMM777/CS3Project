...
public class FirstPersonAIO : MonoBehaviour {
    ...
    private void Start(){
        playerBody = GameObject.Find("Man_03");
        ...
        if (autoCrosshair || drawStaminaMeter){
            ...
            if(drawStaminaMeter){
                ...
                StaminaMeterBG.rectTransform.anchoredPosition = new Vector2(250, 15);
                StaminaMeterBG.rectTransform.sizeDelta = new Vector2(250,6);
                StaminaMeterBG.color = new Color(0,1,0,0);
                StaminaMeter.rectTransform.sizeDelta = new Vector2(250,6);
                StaminaMeter.color = new Color(0,1,0,0);
            }
        }
    ...
    private void FixedUpdate(){
        if (canMove)
        {
            ...
            if(isSprinting){
                ...
                if(drawStaminaMeter){
                    StaminaMeterBG.color = Vector4.MoveTowards(StaminaMeterBG.color, new Vector4(0,1,0, 0.5f),0.15f);
                    StaminaMeter.color = Vector4.MoveTowards(StaminaMeter.color, new Vector4(0,1,0,1),0.15f);
                }
                ...
                if(drawStaminaMeter){
                   if(staminaInternal==staminaLevel){ StaminaMeterBG.color = Vector4.MoveTowards(StaminaMeterBG.color, new Vector4(0, 1, 0, 0),0.15f);
                    StaminaMeter.color = Vector4.MoveTowards(StaminaMeter.color, new Vector4(0, 1, 0, 0),0.15f);}
                            ...
                            if(dynamicFootstep.currentClipSet.Any()) 
                                {
                                    ...
                                    playerBody.GetComponent<FootSteps>().audioSource.PlayOneShot(playerBody.GetComponent<FootSteps>().finalChosen, 1);
                                }
